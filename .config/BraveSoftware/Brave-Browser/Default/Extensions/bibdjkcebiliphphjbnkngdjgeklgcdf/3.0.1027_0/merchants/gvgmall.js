const merchantCfg={
	...
	template:(offer,offersCount,seeOnAKSLink,translates)=> {
		return `
			<li class="proprice"><b>Price</b>
				<div class="old-price">${DOMPurify.sanitize(offer.merchantPriceText)}</div>
				<div class="price">${merchantCfg.priceCleaner(offer.bestOffer.price)}</div>
				<a href="${DOMPurify.sanitize(offer.bestOffer.url)}"${merchantCfg.open_mode} class="btn add_url" style="width:auto;">${translates.buyOnAks}</a>
