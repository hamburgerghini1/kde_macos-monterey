const debug=false;// Enable verbosity in console
...
{		if(!msg.voucher_stats)
		...
		{
			const vs=document.querySelector('#voucherSelection');
			return vs.innerHTML='<h2 style="text-align:center">No coupons here guys sorry.</h2><img src="../images/vouchers/fail.png" height="400" style="display:block;margin:0 auto;">';
		}
		bt.click();
...
function genDevStats()
{
	...
	browser.storage.sync.get().then(
		(value)=>{
			...
			document.querySelector('#dev #storage_sync_content span').innerHTML=JSON.stringify(value).replace('{','{<br>').replace('}','<br>}').replaceAll(',',',<br>');
		}
	);
...
function priceGlobalToggle(e)
{
	...
	console.info('toggle ',e.target.parentNode.parentNode.querySelectorAll('input:first-of-type'),' price to ',value);
	globalMerchantChange=true;
...
function couponGlobalToggle(e)
{
	...
	console.info('toggle ',e.target.parentNode.parentNode.querySelector('input:last-of-type:not(:checked)'),' coupon to ',value);
	globalMerchantChange=true;
...
function init()
{
	...
	for (const [index, merchant] of Object.entries(window.merchants))
	{
		...
		if(lines&&[7,14].includes(lines.length))
